...
import static marvel.MarvelPaths.*;
...
public class MarvelTestDriver {
    ...
    private void loadGraph(String graphName, String fileName) {
        Graph<String,String> newGraph = parseData(fileName);
    ...
    private void findPath(String graphName, String src, String dest) {
        ...
        String srcNode = src.replaceAll("_", " ");
        String destNode = dest.replaceAll("_", " ");
        try{
            if(!newGraph.listNodes().contains(srcNode) || !newGraph.listNodes().contains(destNode)) {
                if(!newGraph.listNodes().contains(srcNode)) {
                    output.println("unknown character "+srcNode);
                }
                if(!newGraph.listNodes().contains(destNode)){
                    output.println("unknown character "+destNode);
                }
            } else{
                List<Graph.graphEdge> path = MarvelPaths.findPath(srcNode,destNode,newGraph);
                output.println("path from "+srcNode+" to " + destNode + ":");
                for(Graph.graphEdge edge: path) {
                    output.println(edge.getNode1()+" to "+edge.getNode2()+ " via "+edge.getLabel());
                }
            }
